...
import (
	...
	crypto "github.com/reapchain/cosmos-sdk/crypto/types"
...
func GenTxCmd(mbm module.BasicManager, txEncCfg client.TxEncodingConfig, genBalIterator types.GenesisBalancesIterator, defaultNodeHome string) *cobra.Command {
	...
	// nolint: exhaustruct
	cmd := &cobra.Command{
		Use:   "gentx [key_name] [amount] [validator type - standing | steering] [eth-address] [orchestrator-address]",
...
`, defaultsDesc, version.AppName,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			orchAddress, err := sdk.AccAddressFromBech32(args[4])
			if err != nil {
				return errors.Wrapf(err, "failed to parse orchAddress(%s)", args[4])
			}
