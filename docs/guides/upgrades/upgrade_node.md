...
First, stop your instance of `mercuryd`. Next, upgrade the software:
...
If the software version does not match, then please check your $PATH to ensure the correct mercuryd is running.
...
echo "export DAEMON_NAME=mercuryd" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.mercuryd" >> ~/.profile
echo 'export PATH="$DAEMON_HOME/cosmovisor/current/bin:$PATH"' >> ~/.profile
source ~/.profile
```


After this, you must make the necessary folders for cosmosvisor in your daemon home directory (~/.mercuryd).
...
There shouldn't be any chain database yet, but in case there is for some reason, you should reset it. This is a good idea especially if you ran `mercuryd start` on an old, broken genesis file.
...
In `~/.mercuryd/config/config.toml` you can set your peers. See the [peers.txt](https://github.com/tharsis/testnets/blob/main/olympus_mons/peers.txt) file for a list of up to date peers.
...
Environment="DAEMON_HOME=$HOME/.mercuryd"
Environment="DAEMON_NAME=mercuryd"
...
To upgrade the genesis file, you can either fetch it from a trusted source or export it locally using the `mercuryd export` command.
