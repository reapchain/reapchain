...
You can use `mercuryd keys` for help with the keys command and `mercuryd keys [command] --help` for more information about a particular subcommand.
...
```bash
mercuryd keys add mykey --keyring-backend test

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(mercuryd keys show mykey -a --keyring-backend test)
```
...
yes $KEYPASSWD | mercuryd keys add me
yes $KEYPASSWD | mercuryd keys show me
# start mercuryd with keyring-backend flag
mercuryd --keyring-backend=file start
```
