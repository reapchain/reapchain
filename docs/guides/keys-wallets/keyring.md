...
You can use `reapchaind keys` for help with the keys command and `reapchaind keys [command] --help` for more information about a particular subcommand.
...
```bash
reapchaind keys add mykey --keyring-backend test

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(reapchaind keys show mykey -a --keyring-backend test)
```
...
yes $KEYPASSWD | reapchaind keys add me
yes $KEYPASSWD | reapchaind keys show me
# start reapchaind with keyring-backend flag
reapchaind --keyring-backend=file start
```
