...
```bash
mercuryd query bank balances $MY_VALIDATOR_ADDRESS --chain-id=evmos_9000-2
```
...
```bash
mercuryd keys add recipient --keyring-backend=file

# Put the generated address in a variable for later use.
RECIPIENT=$(mercuryd keys show recipient -a --keyring-backend=file)
```

The command above creates a local key-pair that is not yet registered on the chain. An account is created the first time it receives tokens from another account. Now, run the following command to send tokens to the `recipient` account:

```bash
mercuryd tx bank send $MY_VALIDATOR_ADDRESS $RECIPIENT 1000000aphoton --chain-id=evmos_9000-2 --keyring-backend=file

# Check that the recipient account did receive the tokens.
mercuryd query bank balances $RECIPIENT --chain-id=evmos_9000-2
```

Finally, delegate some of the stake tokens sent to the `recipient` account to the validator:

```bash
mercuryd tx staking delegate $(mercuryd keys show my_validator --bech val -a --keyring-backend=file) 500aphoton --from=recipient --chain-id=evmos_9000-2 --keyring-backend=file

# Query the total delegations to `validator`.
mercuryd query staking delegations-to $(mercuryd keys show my_validator --bech val -a --keyring-backend=file) --chain-id=evmos_9000-2
```
