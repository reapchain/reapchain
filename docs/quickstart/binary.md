...
`mercuryd` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `mercuryd` binary, run:
...
::: tip
You can also enable auto-completion with the `mercuryd completion` command. For example, at the start of a bash session, run `. <(mercuryd completion)`, and all `mercuryd` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.mercuryd` directory.
...
To specify the `mercuryd` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.mercuryd/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.mercuryd/config/app.toml` file.
...
We can view the default client config setting by using `mercuryd config` command:
...
A list of commonly used flags of `mercuryd` is listed below:

| Option              | Description                   | Type         | Default Value   |
|---------------------|-------------------------------|--------------|-----------------|
| `--chain-id`        | Full Chain ID                 | String       | ---             |
| `--home`            | Directory for config and data | string       | `~/.mercuryd`     |
| `--keyring-backend` | Select keyring's backend      | os/file/test | os              |
| `--output`          | Output format                 | string       | "text"          |

## Command list

A list of commonly used `mercuryd` commands. You can obtain the full list by using the `mercuryd -h` command.
