...
`reapchaind` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `reapchaind` binary, run:
...
::: tip
You can also enable auto-completion with the `reapchaind completion` command. For example, at the start of a bash session, run `. <(reapchaind completion)`, and all `reapchaind` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.reapchaind` directory.
...
To specify the `reapchaind` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.reapchaind/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.reapchaind/config/app.toml` file.
...
We can view the default client config setting by using `reapchaind config` command:
...
```bash
reapchaind config "chain-id" evmos_9000-2
reapchaind config
{
...
A list of commonly used flags of `reapchaind` is listed below:

| Option              | Description                   | Type         | Default Value   |
|---------------------|-------------------------------|--------------|-----------------|
| `--chain-id`        | Full Chain ID                 | String       | ---             |
| `--home`            | Directory for config and data | string       | `~/.reapchaind`     |
| `--keyring-backend` | Select keyring's backend      | os/file/test | os              |
| `--output`          | Output format                 | string       | "text"          |

## Command list

A list of commonly used `reapchaind` commands. You can obtain the full list by using the `reapchaind -h` command.
...
| `reapchain` | Reapchain subcommands   | `show-address`, `show-node-id`, `version`                                 |
