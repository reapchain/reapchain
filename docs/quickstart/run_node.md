...
To run a node with the same key every time: replace `mercuryd keys add $KEY` in `./init.sh` with:
...
::: danger
If you are running a **validator node**, always be careful when doing `mercuryd unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
...
```shell
rate(mercuryd_transactions_processed[1m])
```
