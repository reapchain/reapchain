...
The command above creates all the configuration files needed for your node and validator to run, as well as a default genesis file, which defines the initial state of the network. All these [configuration files](./../../validators/quickstart/binary.md#configuring-the-node) are in `~/.reapchaind` by default, but you can overwrite the location of this folder by passing the `--home` flag.
...
```bash
reapchaind keys add my_validator
```
...
```bash
reapchaind add-genesis-account my_validator 10000000000aevmos
```
