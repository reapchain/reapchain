...
```bash
mercuryd start --json-rpc.enable
```
...
```bash
mercuryd start --json-rpc.api eth,txpool,personal,net,debug,web3,miner
```
...
```bash
# set gas cap to 85M
mercuryd start --json-rpc.gas-cap 85000000000

# set gas cap to infinite (=0)
mercuryd start --json-rpc.gas-cap 0
```
