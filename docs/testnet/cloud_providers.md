...
To connect the node to the existing testnet, fetch the testnet's `genesis.json` file and copy it into the new droplets config directory (i.e `$HOME/.mercuryd/config/genesis.json`).
...
Once the genesis file is copied over run `mercuryd start` inside the node droplet.
