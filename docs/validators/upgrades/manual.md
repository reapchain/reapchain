...
Before upgrading the Evmos version. Stop your instance of `reapchaind` using `Ctrl/Cmd+C`.

Next, upgrade the software to the desired release version. Check the Evmos [releases page](https://github.com/reapchain/reapchain/releases) for details on each release.
...
```bash
rm $HOME/.reapchaind/config/addrbook.json
