...
Find below a list of `reapchaind` commands added with the `x/vesting` module. You can obtain the full list by using the `reapchaind -h` command.
...
```go
reapchaind query vesting balances [address] [flags]
```
...
```go
reapchaind tx vesting create-clawback-vesting-account [to_address] [flags]
```
...
```go
reapchaind tx vesting clawback [address] [flags]
```
