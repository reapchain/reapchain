...
Upon passing, the validator-addresss submitted will be allowed to become a full standing member validator`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("\n=================================================")
			fmt.Println("PERMISSIONS MODULE - CmdRegisterStandingMemberProposal", time.Now().Format(time.RFC822))
			fmt.Println("title: ", title)
			fmt.Println("description: ", description)
			fmt.Println("depositStr: ", depositStr)
			fmt.Println("validatorStr: ", validatorAddresss.String())
			fmt.Println("accountAddress: ", accountAddress.String())
