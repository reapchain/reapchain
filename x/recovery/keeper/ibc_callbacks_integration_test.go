...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("from a non-authorized chain", func() {
		...
		It("should transfer and not recover tokens", func() {
			s.SendAndReceiveMessage(s.pathOsmosisReapchain, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 1)

			nativeReapchain := s.Reapchain.App.(*app.Reapchain).BankKeeper.GetBalance(s.Reapchain.GetContext(), senderAcc, "areap")
			Expect(nativeReapchain).To(Equal(coinReap))
	...
	Describe("from an authorized, non-EVM chain (e.g. Osmosis)", func() {
		Describe("to a different account on Reapchain (sender != recipient)", func() {
			BeforeEach(func() {
				sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
				receiver = s.Reapchain.SenderAccount.GetAddress().String()
				senderAcc, _ = sdk.AccAddressFromBech32(sender)
		...
		Describe("to the sender's own eth_secp256k1 account on Reapchain (sender == recipient)", func() {
			...
			Context("with disabled recovery parameter", func() {
				...
				It("should not transfer or recover tokens", func() {
					s.SendAndReceiveMessage(s.pathOsmosisReapchain, s.IBCOsmosisChain, coinOsmo.Denom, coinOsmo.Amount.Int64(), sender, receiver, 1)
			...
			Context("with a sender's claims record", func() {
				...
				Context("with completed actions", func() {
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						err := s.pathOsmosisReapchain.RelayPacket(CreatePacket("10000", "areap", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						err = s.pathOsmosisReapchain.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 2, timeout))
						s.Require().NoError(err)

						// Check that the areap were recovered
						nativeReapchain := s.Reapchain.App.(*app.Reapchain).BankKeeper.GetBalance(s.Reapchain.GetContext(), senderAcc, "areap")
						Expect(nativeReapchain.IsZero()).To(BeTrue())
						ibcReapchain := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, areapIbcdenom)
						Expect(ibcReapchain).To(Equal(sdk.NewCoin(areapIbcdenom, coinReap.Amount)))
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					BeforeEach(func() {
						...
						s.SendAndReceiveMessage(s.pathCosmosReapchain, s.IBCCosmosChain, coinAtom.Denom, coinAtom.Amount.Int64(), s.IBCCosmosChain.SenderAccount.GetAddress().String(), receiver, 1)
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						s.SendAndReceiveMessage(s.pathOsmosisReapchain, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 2)

						timeout = uint64(s.Reapchain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
						err = s.pathOsmosisReapchain.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 3, timeout))
						s.Require().NoError(err)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						...
						transferMsg := transfertypes.NewMsgTransfer(s.pathOsmosisReapchain.EndpointA.ChannelConfig.PortID, s.pathOsmosisReapchain.EndpointA.ChannelID, sdk.NewCoin(uatomIbcdenom, sdk.NewInt(10)), sender, receiver, timeoutHeight, 0)
						...
						packet := channeltypes.NewPacket(transfer.GetBytes(), 1, s.pathOsmosisReapchain.EndpointA.ChannelConfig.PortID, s.pathOsmosisReapchain.EndpointA.ChannelID, s.pathOsmosisReapchain.EndpointB.ChannelConfig.PortID, s.pathOsmosisReapchain.EndpointB.ChannelID, timeoutHeight, 0)
						// Receive message on the reapchain side, and send ack
						err = s.pathOsmosisReapchain.RelayPacket(packet)
						s.Require().NoError(err)
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						err := s.pathOsmosisReapchain.RelayPacket(CreatePacket("10000", "areap", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						err = s.pathOsmosisReapchain.RelayPacket(CreatePacket("10", "transfer/channel-0/transfer/channel-1/uatom", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 2, timeout))
						s.Require().NoError(err)
						err = s.pathOsmosisReapchain.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 3, timeout))
						s.Require().NoError(err)
