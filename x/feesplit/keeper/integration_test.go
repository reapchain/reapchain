...
var _ = Describe("Fee distribution:", Ordered, func() {
	...
	BeforeAll(func() {
		...
		params = s.app.FeesplitKeeper.GetParams(s.ctx)
		params.EnableFeeSplit = true
		s.app.FeesplitKeeper.SetParams(s.ctx, params)
	...
	Context("with feesplit param disabled", func() {
		...
		BeforeAll(func() {
			...
			fee, isRegistered := s.app.FeesplitKeeper.GetFeeSplit(s.ctx, registeredContract)
		...
		It("should not allow new contract registrations", func() {
			...
			Expect(
				strings.Contains(res.GetLog(),
					"feesplit module is disabled by governance"),
			).To(BeTrue())
			s.Commit()

			_, isRegistered := s.app.FeesplitKeeper.GetFeeSplit(s.ctx, contractAddress)
		...
		It("should not allow fee split updates for previously registered contracts", func() {
			...
			msg := types.NewMsgUpdateFeeSplit(
				registeredContract,
				deployerAddress,
				withdrawerAddress,
			)
		...
		It("should not allow cancellations of previously registered contracts", func() {
			msg := types.NewMsgCancelFeeSplit(registeredContract, deployerAddress)
			res := deliverTx(deployerKey, nil, msg)
	...
	Context("with feesplit param enabled", func() {
		BeforeEach(func() {
		...
		Describe("Registering a contract for receiving tx fees", func() {
			Context("with an empty withdrawer address", Ordered, func() {
				...
				It("should be possible", func() {
					...
					fee, isRegistered := s.app.FeesplitKeeper.GetFeeSplit(s.ctx, contractAddress)
					Expect(isRegistered).To(Equal(true))
		...
		Describe("Canceling a fee split registration", func() {
			When("the registered fee split exists", Ordered, func() {
				...
				It("should be possible", func() {
					msg := types.NewMsgCancelFeeSplit(contractAddress, deployerAddress)
					res := deliverTx(deployerKey, nil, msg)
...
func registerFee(
	...
	msg := types.NewMsgRegisterFeeSplit(*contractAddress, deployerAddress, withdrawerAddress, nonces)
	...
	if res.IsOK() {
		registerEvent := res.GetEvents()[8]
		Expect(registerEvent.Type).To(Equal(types.EventTypeRegisterFeeSplit))
