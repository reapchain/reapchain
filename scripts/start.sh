...
init_func() {
    ...
    "$PWD"/build/mercuryd keys add $KEY"$i" --home "$DATA_DIR$i" --no-backup --chain-id $CHAINID --algo "eth_secp256k1" --keyring-backend test
    echo "init Evmos with moniker=$MONIKER and chain-id=$CHAINID"
    "$PWD"/build/mercuryd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    echo "prepare genesis: Allocate genesis accounts"
    "$PWD"/build/mercuryd add-genesis-account \
    "$("$PWD"/build/mercuryd keys show "$KEY$i" -a --home "$DATA_DIR$i" --keyring-backend test)" 1000000000000000000aphoton,1000000000000000000stake \
    --home "$DATA_DIR$i" --keyring-backend test
    echo "prepare genesis: Sign genesis transaction"
    "$PWD"/build/mercuryd gentx $KEY"$i" 1000000000000000000stake --keyring-backend test --home "$DATA_DIR$i" --keyring-backend test --chain-id $CHAINID
    echo "prepare genesis: Collect genesis tx"
    "$PWD"/build/mercuryd collect-gentxs --home "$DATA_DIR$i"
    echo "prepare genesis: Run validate-genesis to ensure everything worked and that the genesis file is setup correctly"
    "$PWD"/build/mercuryd validate-genesis --home "$DATA_DIR$i"
}

start_func() {
    echo "starting evmos node $i in background ..."
    "$PWD"/build/mercuryd start --pruning=nothing --rpc.unsafe \
