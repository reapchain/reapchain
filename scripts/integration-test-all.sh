...
init_func() {
    ...
    "$PWD"/build/reapchaind validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    REAPCHAIN_PID=$!
    echo "started reapchain node, pid=$REAPCHAIN_PID"
    # add PID to array
    arr+=("$REAPCHAIN_PID")
...
stop_func() {
    REAPCHAIN_PID=$i
    echo "shutting down node, pid=$REAPCHAIN_PID ..."

    # Shutdown reapchain node
    kill -9 "$REAPCHAIN_PID"
    wait "$REAPCHAIN_PID"
